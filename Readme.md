...
*   **服务器监控与控制**：
    ...
    *   仪表盘显示 CPU、内存、运行时长、当前服务端版本和实时在线人数/人数上限。
    *   在线人数由面板在同一容器内通过 `127.0.0.1:<DefaultPort>` 调用游戏原生 Steam A2S_INFO 查询，不依赖日志、不需要 RCON 密码，也不需要新增 Docker 端口映射。
    *   提供“重启”和“更新并重启”功能（自动触发 SteamCMD 更新）。
    *   提供面板自重启功能，方便build调试

*   **备份与恢复**：
    *   “监控维护”页面支持手动备份、按小时配置定时备份、备注、保留数量上限、列表管理、删除和恢复；定时备份开关、间隔、保留数量和游戏内备份参数修改后自动保存，也可点击立即保存。
